...
router.post('/get_all_productdetail_by_productid', function (req, res, next) {
  try {
    pool.query("select PD.*, (select P.productname from product P where P.productid=PD.productid) as productname from productdetail PD where PD.productid=?", [req.body.productid], function (error, result) {
      if (error) {
        console.log(error)
