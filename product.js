...
router.get("/display_all_product", function (req, res) {
  try {
    pool.query(
      "select P.*,(select C.categoryname from category C where C.categoryid=P.categoryid) as categoryname,(select SC.subcategoryname from subcategory SC where SC.subcategoryid=P.subcategoryid) as subcategoryname,(select B.brandname from brands B where B.brandid=P.brandid) as brandname  from product P",
      function (error, result) {
        if (error) {
          res.status(200).json({
            message: "Database Error Pls contact with backend team...!",
...
router.post('/get_all_product_by_brandid', function (req, res, next) {
  try {
    pool.query("select P.*, (select B.brandname from brands B where B.brandid=P.brandid) as brandname from product P where P.brandid=?", [req.body.brandid], function (error, result) {
      if (error) {
        console.log(error)
